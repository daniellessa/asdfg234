...
public class CalendarView extends View {
    ...
    private List<? extends ViewEvent> mPreviousPeriodEvents;
    private List<? extends ViewEvent> mCurrentPeriodEvents;
    private List<? extends ViewEvent> mNextPeriodEvents;
    ...
    private int teste = 18;

    // Listeners.
    private EventClickListener mEventClickListener;
    private EventLongPressListener mEventLongPressListener;
    private ViewLoader mWeekViewLoader;
    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * teste + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, 0, (int) velocityY, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * teste + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - getHeight()), 0);
                    break;
            }
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / teste));
        ...
        if (mCurrentOrigin.y < getHeight() - mHourHeight * teste - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2)
            mCurrentOrigin.y = getHeight() - mHourHeight * teste - mHeaderTextHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom - mTimeTextHeight/2;
        ...
        for (int dayNumber = leftDaysWithGaps + 1;
            ...
            int i = 0;
            for (int hourNumber = 0; hourNumber < teste; hourNumber++) {
        ...
        if (mEventRects != null && mEventRects.size() > 0) {
            for (int i = 0; i < mEventRects.size(); i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date)) {
                    ...
                    // Calculate top.
                    float top = mHourHeight * teste * mEventRects.get(i).top / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 + mEventMarginVertical;

                    // Calculate bottom.
                    float bottom = mEventRects.get(i).bottom;
                    bottom = mHourHeight * teste * bottom / 1440 + mCurrentOrigin.y + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom + mTimeTextHeight/2 - mEventMarginVertical;
     ...
     */
    private void drawEventTitle(ViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mWeekViewLoader != null){
            ...
            if (!isInEditMode() && (mFetchedPeriod < 0 || mFetchedPeriod != periodToFetch || mRefreshEvents)) {
                List<? extends ViewEvent> previousPeriodEvents = null;
                List<? extends ViewEvent> currentPeriodEvents = null;
                List<? extends ViewEvent> nextPeriodEvents = null;
     ...
     */
    private void cacheEvent(ViewEvent event) {
        ...
        if (!isSameDay(event.getStartTime(), event.getEndTime())) {
            ...
            ViewEvent event1 = new ViewEvent(event.getId(), event.getName(), event.getLocation(), event.getStartTime(), endTime);
            ...
            while (!isSameDay(otherDay, event.getEndTime())) {
                ...
                ViewEvent eventMore = new ViewEvent(event.getId(), event.getName(), overDay, endOfOverDay);
            ...
            ViewEvent event2 = new ViewEvent(event.getId(), event.getName(), event.getLocation(), startTime, event.getEndTime());
     ...
     */
    private void sortAndCacheEvents(List<? extends ViewEvent> events) {
        sortEvents(events);
     ...
     */
    private void sortEvents(List<? extends ViewEvent> events) {
        Collections.sort(events, new Comparator<ViewEvent>() {
            @Override
            public int compare(ViewEvent event1, ViewEvent event2) {
     ...
     */
    private boolean isEventsCollide(ViewEvent event1, ViewEvent event2) {
        long start1 = event1.getStartTime().getTimeInMillis();
        long end1 = event1.getEndTime().getTimeInMillis();
        long start2 = event2.getStartTime().getTimeInMillis();
        long end2 = event2.getEndTime().getTimeInMillis();
     ...
     */
    public ViewLoader getWeekViewLoader(){
     ...
     */
    public void setWeekViewLoader(ViewLoader loader){
    ...
    public void goToHour(double hour){
        ...
        if (hour > teste)
            verticalOffset = mHourHeight * teste;
        else if (hour > 0)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * teste - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * teste - getHeight() + mHeaderTextHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
